...
public final class SunPKCS11 extends AuthProvider {
    ...
    byte[] exportKey(long hSession, CK_ATTRIBUTE[] attributes, long keyId) throws PKCS11Exception {
        ...
        try {
            ...
            // wrapKey = (P11Key)P11Key.secretKey(wrapKeyGenSession, genKeyId, "AES", 256 >> 3, new CK_ATTRIBUTE[] {new CK_ATTRIBUTE(CKA_WRAP, true)});
            // CK_ATTRIBUTE[] tmpAttributes = P11Key.getAttributes(wrapKeyGenSession, genKeyId, null, new CK_ATTRIBUTE[] {
            //     new CK_ATTRIBUTE(CKA_TOKEN),
            //     new CK_ATTRIBUTE(CKA_SENSITIVE),
            //     new CK_ATTRIBUTE(CKA_EXTRACTABLE),
            // });
            // wrapKey = new P11Key.P11SecretKey(wrapKeyGenSession, genKeyId, "AES", 256 >> 3, tmpAttributes);
        } catch (PKCS11Exception e) {
            throw e;
        } finally {
            ...
            if(keyType == CKK_RSA) {
                try {
                    keyBytes = RSAPrivateCrtKeyImpl.newKey(
                        KeyType.RSA,
                        null,
                        ckAttrsMap.getOrDefault(CKA_MODULUS, new CK_ATTRIBUTE(CKA_MODULUS, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_PUBLIC_EXPONENT, new CK_ATTRIBUTE(CKA_PUBLIC_EXPONENT, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_PRIVATE_EXPONENT, new CK_ATTRIBUTE(CKA_PRIVATE_EXPONENT, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_PRIME_1, new CK_ATTRIBUTE(CKA_PRIME_1, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_PRIME_2, new CK_ATTRIBUTE(CKA_PRIME_2, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_EXPONENT_1, new CK_ATTRIBUTE(CKA_EXPONENT_1, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_EXPONENT_2, new CK_ATTRIBUTE(CKA_EXPONENT_2, BigInteger.ZERO)).getBigInteger(),
                        ckAttrsMap.getOrDefault(CKA_COEFFICIENT, new CK_ATTRIBUTE(CKA_COEFFICIENT, BigInteger.ZERO)).getBigInteger()
                    ).getEncoded();
